...
def html_form(func):
    ...
    try:
        ...
        for arg_name, default in args.items():
            ...
            ordered_form['args'][arg_name] = {'desc': arg_name.replace('_', ' '), 'input': 'text', 'default': '', 'status': ''}
            ordered_form['args'][arg_name]['default'] = default
    ...
    if config.VENV_NAME == 'idc':
        logger.debug('idc venv: need confirm when submit the form')
