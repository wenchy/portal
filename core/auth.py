...
def auth_api(handler):
    ...
    if appid and appid in config.AUTHS["apis"]:
        ...
        logger.debug("encoded = {} | signature = {}".format(encoded, signature))
        ...
        def wrap_execute(handler_execute):
            ...
            def _execute(self, transforms, *args, **kwargs):
                for name, value in self.request.headers.get_all():
                    logger.debug("headers: " + name + ':' + value)
                ...
                if ok:
                    logger.info('success|username: %s|specified auth: %s|real auth: %s|class: %s|arguments: %s' % (username, auth_type, real_auth_type, handler_class.__name__, str(self.request.arguments)))
                    kwargs['username'] = username
                    kwargs['auth_type'] = real_auth_type
                    return handler_execute(self, transforms, *args, **kwargs)
                else:
                    logger.info('username: %s|auth failed: %s|class: %s|arguments: %s' % (username, auth_type, handler_class.__name__, str(self.request.arguments)))
                    self.write('<h3>Permission denied!</h3>')
                    self.write('Please contact the backend dev-group if you do need permission.')
